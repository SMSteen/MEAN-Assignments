        ...
        User.findOne({ email })
            .then(user => {
                ...
                return User.validatePassword(password, user.password).then(result => {
                    if(result) { //passwords matched, proceed with login
                        handleLogin(request, response, user);
                    } else { //passwords do not match, throw error
                        throw Error();
                    }
                });
            })
            .catch(error => {
                response.status(404).json("You have entered an invalid email address or password.")
            })
    },
